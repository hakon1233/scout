...
function FeedFilter({
  ...
  React.useEffect(() => {
    ...
    function onKeyDown(e: KeyboardEvent) {
      // Disclosure semantics (CAR-245): Escape closes the popover and returns
      // focus to the trigger so keyboard users aren't dropped to <body>.
      if (e.key === "Escape") {
        setOpen(false);
...
function ProfileMenu({
  ...
  // Close on outside click or Escape while open. CAR-245: this popover is a
  // disclosure (Tab-navigated content, not an ARIA application menu), so Escape
  // closes it and returns focus to the trigger rather than leaving focus on a
  // removed node.
  React.useEffect(() => {
    if (!open) return;
    function onPointerDown(e: MouseEvent) {
