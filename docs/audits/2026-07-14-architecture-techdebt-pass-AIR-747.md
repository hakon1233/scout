...
finding one stale issue reference in the prior doc worth correcting.
...
un-tracked; every standing item maps to an already-open issue (or a deliberately
**cancelled** one). Corrected a stale reference: the prior doc's `AIR-198`
(shared `spawnClaude` extraction) is actually tracked by **AIR-661**, now
**cancelled** — so that item is no longer open debt and must not be re-filed.
...
|---|---|---|
| Extract shared `spawnClaude()` (research/chat spawn dup) | ~~AIR-198~~ **AIR-661** | **cancelled** — no longer open debt (prior doc's AIR-198 ref was stale) |
...
- **Web↔agent wire types are hand-copied** (`AgentBrief`, chat wire types, brief
  ...
  solo-pass fix.
- **`spawnClaude` dedup is now un-tracked (AIR-661 cancelled).** The duplication
  between `chat.ts` and `research.ts` still exists; someone deliberately declined
  to pursue the extraction. Do **not** silently re-file it — if it resurfaces as a
  real problem, reopen AIR-661 with the new evidence rather than opening a fresh
  duplicate.
...
correcting a stale issue reference and confirming the `spawnClaude` item is
deliberately closed rather than dropped. The remaining tracked items are all
